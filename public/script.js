...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const waveformBars = waveformEl ? Array.from(waveformEl.querySelectorAll('.bar')) : [];
    ...
    function applyMobileMarquee() {
        if (window.innerWidth > 768) return;
        const selector = '.song-item .song-title, .recommend-item .song-title, .mini-title, .track-name';
        const nodes = document.querySelectorAll(selector);
        nodes.forEach((el) => {
            if (!el) return;
            el.classList.remove('song-title-marquee');
            const parent = el.parentElement || el;
            const containerWidth = parent.clientWidth;
            if (!containerWidth) return;
            const textWidth = el.scrollWidth;
            if (textWidth > containerWidth + 1) {
                el.classList.add('song-title-marquee');
            }
        });
    }

    window.addEventListener('resize', applyMobileMarquee);
    window.addEventListener('orientationchange', applyMobileMarquee);
    ...
    function resetMobilePlaylistPosition() {
        if (!sideDrawer) return;
        const drawerPlaylistSection = sideDrawer.querySelector('.drawer-playlist-section');
        if (!drawerPlaylistSection) return;
        sideDrawer.classList.remove('playlist-fullscreen');
        drawerPlaylistSection.classList.remove('is-expanded');
        drawerPlaylistSection.classList.add('is-collapsed');
    }

    function toggleSideDrawer() {
        sideDrawer.classList.toggle('visible');
        if (sideDrawer.classList.contains('visible')) {
            resetMobilePlaylistPosition();
        }
    }
    ...
    if (sideDrawer) {
        ...
        const drawerPlaylistSection = sideDrawer.querySelector('.drawer-playlist-section');
        if (drawerPlaylistSection) {
            drawerPlaylistSection.classList.add('is-collapsed');
            drawerPlaylistSection.addEventListener('click', (e) => {
                const rect = drawerPlaylistSection.getBoundingClientRect();
                const offsetY = e.clientY - rect.top;
                if (offsetY > 56) return;
                e.stopPropagation();
                const isCollapsed = drawerPlaylistSection.classList.contains('is-collapsed');
                drawerPlaylistSection.classList.toggle('is-collapsed', !isCollapsed);
                drawerPlaylistSection.classList.toggle('is-expanded', isCollapsed);
                if (isCollapsed) {
                    sideDrawer.classList.add('playlist-fullscreen');
                } else {
                    sideDrawer.classList.remove('playlist-fullscreen');
                }
            });
    ...
    */
    ...
    function updatePlayerInfo(item) {
        ...
        if (bigCover && bigCover.tagName !== 'IMG') {
            ...
            if (!newImg.className) newImg.className = 'player-cover';
        ...
        if (bigCover) {
             bigCover.removeAttribute('data-has-error');
             if (item.artwork) {
    ...
    function ensureVisualizerReady() {
        if (audioCtx || !audio || !waveformBars.length) return;
    ...
    function startVisualizer() {
        ...
        const hasAnalyser = !!(audioCtx && analyser && analyserDataArray);
        const bufferLength = hasAnalyser ? analyser.frequencyBinCount : 0;
        const step = hasAnalyser && barCount ? Math.max(1, Math.floor(bufferLength / barCount)) : 1;
        const minHeight = 4;
        const maxHeight = 28;
        ...
        function draw() {
            ...
            if (hasAnalyser) {
                ...
                for (let i = 0; i < bufferLength; i++) {
                    total += analyserDataArray[i];
                }
            }
            for (let i = 0; i < barCount; i++) {
                let ratio;
                if (hasAnalyser && total > 0) {
                    const index = i * step;
                    const value = analyserDataArray[index] || 0;
                    ratio = value / 255;
                } else {
                    const phase = (frame / 6) + (i / 2);
